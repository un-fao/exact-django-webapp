...
class PerennialToRiceWithOrganicSoil(t.LandUseChangeTest):
    def __init__(self):
        super().__init__()
        ...
        self.module_end: FloodedRice = self.create_module(self.module_type_w, land_use_change=self.land_use_change)

        # Add organic soil module
        self.module_type_organic_soil = ModuleType.objects.get(class_name="OrganicSoil")
        self.add_activity_modules([self.module_type_organic_soil])
        self.organic_soil: OrganicSoil = self.create_module(self.module_type_organic_soil, land_use_change=self.land_use_change)

    def test(self):
        self.calculate_results()


PerennialToRiceWithOrganicSoil().test()
