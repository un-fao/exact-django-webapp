...
class LivestockTestCase(base_module.BaseModuleTestCase):
    def setUp(self):
        self.ModuleClass = models.Livestock
        super().setUp()
        ...
        self.validated_data = {
            "livestock_category_type": models.LivestockCategoryType.objects.order_by("?").first().id,
            "livestock_production_type_start": models.LivestockProductionType.objects.order_by("?").first().id,
            "livestock_production_type_w": models.LivestockProductionType.objects.order_by("?").first().id,
            "livestock_production_type_wo": models.LivestockProductionType.objects.order_by("?").first().id,
            "production_start": FuzzyFloat(0, 100).fuzz(),
            "production_w": FuzzyFloat(0, 100).fuzz(),
            "production_wo": FuzzyFloat(0, 100).fuzz(),
            "heads_number_start": FuzzyInteger(0, 100).fuzz(),
            "heads_number_w": FuzzyInteger(0, 100).fuzz(),
            "heads_number_wo": FuzzyInteger(0, 100).fuzz(),
            "complementary_manure_management_start": models.ManureManagementType.objects.order_by("?").first().id,
            "complementary_manure_management_w": models.ManureManagementType.objects.order_by("?").first().id,
            "complementary_manure_management_wo": models.ManureManagementType.objects.order_by("?").first().id,
        }
    ...
    def test_modify_and_check_cache_invalidation(self):
        ...
        validated_data = copy.deepcopy(self.validated_data)
        validated_data["heads_number_w"] = 100000
        response = self.edit_module(self.module, self.user, validated_data)
