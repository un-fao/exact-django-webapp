...
def GHG_emissions(
    ...
    bgb_t_dm_per_ha_default = (
        bgb_t_dm_per_ha_default_input_parameter * agb_t_dm_per_ha_default
    )

    agb_t_c_per_ha = (
        agb_t_dm_per_ha_default * mangrove_factor
        if not agb_t_c_per_ha_tier_2
        else agb_t_c_per_ha_tier_2
    )
    bgb_t_c_per_ha = (
        bgb_t_dm_per_ha_default * mangrove_factor
        if not bgb_t_c_per_ha_tier_2
        else bgb_t_c_per_ha_tier_2
    )

    hwp_before_t_c_per_ha = (
        agb_t_dm_per_ha_default * mangrove_factor
        if hwp_before_t_dm_per_ha > agb_t_dm_per_ha_default
        else hwp_before_t_dm_per_ha * mangrove_factor
    )

    if dw_tier_2:
        dw = dw_tier_2
    if litter_tier_2:
        litter = litter_tier_2
    ...
    delta_c_mineral_per_ha = (
        ...
        else soc_after_defo_tier_2 - soc_reference
    )

    fire_t_dm_per_ha = (
        agb_t_dm_per_ha_default - hwp_before_t_dm_per_ha if fire_bool else 0
    )
    # THIS IS CORRECT

    biomass_forest_agb_bgb_t_c_per_ha = (
        agb_t_c_per_ha + bgb_t_c_per_ha - hwp_before_t_c_per_ha
    )
    ...
    som_luc_kg_n_per_year = (
        ...
        else -(delta_c_mineral_per_ha / (20 * c_n_ratio)) * 1000
    )

    n2o_n_conversion_factor = 44 / 28
    soil_kg_n2o = (
        moisture_emission_factor * n2o_n_conversion_factor * som_luc_kg_n_per_year
    )
    ...
    if biomass_final_1_year_t_per_ha_tier_2:
        biomass_final_1_year_t_per_ha = biomass_final_1_year_t_per_ha_tier_2

    biomass_forest_dom_t_co2_per_ha = biomass_forest_dom_t_c_per_ha * (44 / 12)

    total_ch4_n2o_per_ha = (
        fire_kg_ch4 * methane + (fire_kg_n2o + soil_kg_n2o) * nitrous
    ) / 1000
    ...
    biomass_gain_wo = (
        0
        if biomass_loss_wo == 0
        else -(biomass_final_1_year_t_per_ha * area_defo_wo * (44 / 12))
    )
    biomass_gain_w = (
        0
        if biomass_loss_w == 0
        else -(biomass_final_1_year_t_per_ha * area_defo_w * (44 / 12))
    )

    dom_loss_wo = biomass_forest_dom_t_co2_per_ha * area_defo_wo
    dom_loss_w = biomass_forest_dom_t_co2_per_ha * area_defo_w

    soil_wo = soil_emissions(
        time_impl,
        time_cap,
        rate_type_soil,
        rate_of_change_soil,
        area_defo_wo,
        delta_co2_mineral_per_ha_per_yr,
        delta_c_mineral_per_ha,
    )
    soil_w = soil_emissions(
        time_impl,
        time_cap,
        rate_type_soil,
        rate_of_change_soil,
...
def soil_emissions(
    ...
    comparison_term_1 = (
        ...
        if rate_type == "I"
            ...
            delta_co2_mineral_per_ha_per_yr
            * soil_rate_other(
                time_tot, time_impl, area_defo, rate_of_change_soil, rate_type
            )
        )
    )
...
def soil_rate_other(time_tot, time_impl, area_defo, rate_of_change_soil, rate_type):
    return area_defo * middle_if_other(
        time_tot, time_impl, rate_of_change_soil, rate_type
    )
...
def middle_if_other(time_tot, time_impl, rate_of_change_soil, rate_type):
    if time_tot > time_impl + 20:
        ...
        if time_tot < 20:
            ...
            return (
                min(20, time_impl) * rate_of_change_soil
                + time_tot
                - min(20, time_impl)
                - outer_if_other(rate_type, time_tot, time_impl)
            )
...
def outer_if_other(rate_type, time_tot, time_impl):
    if rate_type == "E":
        return (
            time_tot
            - 20
            + 0.217 * time_impl * (math.exp(4.606 * (20 - time_tot / time_impl)) - 1)
        )
    else:
