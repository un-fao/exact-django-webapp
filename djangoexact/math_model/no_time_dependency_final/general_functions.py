...
def compute_yearly_or_half_year_cumulative(start_value, end_value, years_implementation, years_capitalization, function, interim_values=True):

    # IMPORTANT NOTE:
    # IF WE SET interim_values to TRUE UTILIZING THIS FUNCTION WE ARE RETURNING THE MIDDLE VALUE BETWEEN THE CALCULATED VALUES AT THE BEGINNING OF EACH YEAR.
...
def compute_half_year_delta(start_value, end_value, years_implementation, years_capitalization, function):
    result_interim = compute_yearly_or_half_year_cumulative(start_value, end_value, years_implementation, years_capitalization, function, interim_values=True)
    result_not_interim = compute_yearly_or_half_year_cumulative(start_value, end_value, years_implementation, years_capitalization, function, interim_values=False)
...
def compute_yearly_cumulative_matrix(start_value, end_value, years_implementation, years_capitalization, function, interim_values=True):

    if function == "linear":
        years_total = years_implementation + years_capitalization

        half_year = compute_half_year_delta(start_value, end_value, years_implementation, years_capitalization, function)
        full_year = compute_yearly_delta(start_value, end_value, years_implementation, years_capitalization, function)
...
def compute_matrix_for_log_rec_dis(start_value, end_value, years_implementation, years_capitalization, function, interim_values=True):
...
def gas_head_calculation(tam: float, vser_or_ner: float, ef_prp: float, 
    ...
    try:
        ...
        if gas_system_tier_2 is None:
            # NOTE: When single system is provided, we assume that the percentage of the other system is
            # 100 - percentage_prp_default, as it is the only other system in the calculation
            if len(percentage_system_default) == 1:
                percentage_system_default = [100-percentage_prp_default]
...
def soil_emissions(soc_start, soc_end, total_hectares, area_start, area_end, hectares_before_20):
    ...
    emissions_soil_yearly = breakdown_proportionally_to_values(total, hectares_before_20)
...
def som_emissions(soc_final, soc_initial, emission_factor_nitrous, nitrous_constant, hectares_before_20):
    ...
    emissions_som_yearly = breakdown_proportionally_to_values(total, hectares_before_20)
...
def biomass_emissions(
    ...
    biomass_emissions_yearly = breakdown_proportionally_to_values(biomass_emissions_total, yearly_variation_hectares)
...
def input_single_calculation(unit_start, unit_end, ipcc_factor, tier_2_factor, unit_factor, emissions_factor, time_implementation, time_capitalization, rate_type):
    try:
        ...
        annual_emissions = compute_yearly_or_half_year_cumulative(emissions_start, emissions_end, time_implementation, time_capitalization, rate_type)
...
def soil_emissions_delta_soc_known(delta_soil_c, delta_soil_c_20_years, area_start, area_end, hectars_before_20):
    ...
    emissions_soil_yearly = breakdown_proportionally_to_values(emissions, hectars_before_20)
