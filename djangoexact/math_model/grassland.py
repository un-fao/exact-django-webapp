...
def residue_burning(area_start, area, time_impl, time_cap, rate_coefficient, nitrous_constant, methane_constant, fire_interval, 
    ...
    if time_impl + time_cap < fire_interval or not fire_used:
        ...
        total = (min(area_start, area) * (time_cap + time_impl) + abs(area - area_start) * func1(area_start, area, rate_coefficient, time_impl, time_cap)) * annual_co2

        return total


residue_burning(50, 50, 20, 9, 0.5, 265, 28, 5, True, 2.3,0.21 , 6.2, None, 0.77, None)

ao = calculate_soil_emissions(20,9,'D',0.5, 50, 50, 53.82, None, 32.2, None)
