#!/usr/bin/env python3
"""
Script to aggregate livestock.csv data by environmental factors:
...
def aggregate_by_environmental_factors(data, headers):
    ...
    if any(idx is None for idx in [module_type_idx, region_idx, climate_idx, moisture_idx, soil_type_idx]):
        missing = []
        if module_type_idx is None:
    ...
    for row in data:
        if len(row) > max(module_type_idx, region_idx, climate_idx, moisture_idx, soil_type_idx):
            try:
                # Create a key combining all environmental factors
                module_type = row[module_type_idx]
                ...
                key = (module_type, region, climate, moisture, soil_type)
...
def run():
    ...
    output_dir = script_dir / "aggregated_data" / "livestock"
    ...
    try:
        ...
        with open(output_path, "w", newline="", encoding="utf-8") as file:
            ...
            writer.writerow(["module_type", "region", "climate", "moisture", "soil_type", "count", "sum", "mean", "median", "min", "max", "q1", "q3"])  # Write header
            for (module_type, region, climate, moisture, soil_type), stats in sorted_data:
                writer.writerow([module_type, region, climate, moisture, soil_type, stats["count"], stats["sum"], stats["mean"], stats["median"], stats["min"], stats["max"], stats["q1"], stats["q3"]])
        ...
        for i, ((module_type, region, climate, moisture, soil_type), stats) in enumerate(sorted_data[:10], 1):
            print(f"    {i}. {module_type} | {region} | {climate} | {moisture} | {soil_type}")
        ...
        print(f"\n  - Statistics:")
        print(f"    - Number of unique module types: {len(set(module_type for (module_type, _, _, _, _), _ in sorted_data))}")
