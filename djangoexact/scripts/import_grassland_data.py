#!/usr/bin/env python3
"""
Django management command to import aggregated grassland data into EmissionStatisticsByModule model.
...
def import_aggregated_module_data():
    ...
    csv_file = script_dir / "aggregated_data" / "grassland" / "aggregated_by_environmental_factors.csv"

    if not csv_file.exists():
        print(f"Error: CSV file not found at {csv_file}")
        return

    print(f"Reading aggregated grassland data from {csv_file}...")
    ...
    try:
        with open(csv_file, "r", encoding="utf-8") as file:
            ...
            for row in reader:
                # Extract environmental factors (note: grassland CSV has 'module' not 'module_type')
                module_type = row["module"]  # Map 'module' from CSV to 'module_type' for model
                region = row["region"]
                climate = row["climate"]
...
def run():
    ...
    """
    print("Starting import of aggregated grassland environmental factors data...")
