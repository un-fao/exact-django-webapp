...
Last activity: 2026-08-10 - Completed quick task 260810-nlo: project-level results cache with stamp-based invalidation and per-edit async recompute on branch feat/project-results-cache
...
| 260810-nlo | Added a project-level results cache so GET /api/projects/{id}/results/ answers from a stored payload instead of reassembling it per call. New ProjectResultCache model plus a monotonic Project.results_stamp bumped atomically from invalidate_module_caches, and the two previously uncovered paths (top-level Module.delete, Activity.delete). Cache key folds project pk, stamp, sorted parsed activity ids and a schema version, so a stale row is never read and no delete sweep is needed. Read path stays synchronous compute-on-miss returning 200, permission check strictly above the cache read, so the frontend contract is untouched. Async recompute reuses the existing AsyncJob plus Cloud Run job path (new RESULTS_RECOMPUTE kind, migration 0291) with per-edit enqueue guarded by PENDING/RUNNING dedupe and stamp-supersede detection, dispatched via transaction.on_commit and gated by RESULTS_RECOMPUTE_ENABLED. No new packages, no Redis, no broker. The Cloud Scheduler sweep debounce was rejected because no cron.yaml is checked in. Confirmed as a product rule during this task: load_reference_data invalidating nothing is intended, not a defect, because an untouched project must be preserved exactly as the user last computed it, so the cache key folds in no reference-data epoch and the rule is recorded as an engineering note in BACKLOG.md. The copy_activity deep-copy of a valid cache is a genuine bug and stays filed as P2; scripts/invalidate_results_cache.py is the deliberate operator escape hatch | 2026-08-10 | efea7f94 | [260810-nlo-cache-project-results-with-invalidation-](./quick/260810-nlo-cache-project-results-with-invalidation-/) |
